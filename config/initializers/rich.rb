...
if Object.const_defined?("Rich")
  Rich.setup do |config|    
    ...
    config.editor[:startupOutlineBlocks] = false
    config.editor[:contentsLanguage] = "es"
    config.editor[:disableNativeSpellChecker] = false
    config.editor[:toolbar_Basic] = [ [ "Bold", "Italic" ] ]
    config.editor[:toolbar] = "Basic"
    config.editor[:height] = "200px"
    config.editor[:width] = "50%"
